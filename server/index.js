...
const upload = multer({
  storage,
  limits: {
    fileSize: 100 * 1024 * 1024, // Change from 50MB to 100MB
  },
...
app.get("/debug/posts", async (req, res) => {
  try {
    ...
    const posts = await Post.find().select(
      "firstName lastName userPicturePath picturePath videoPath description"
    );
    console.log("All posts with media paths:", posts);
    res.json(posts);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
